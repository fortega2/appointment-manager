...
// SetRetryAfter writes Retry-After (RFC 9110 §10.2.3) in delta-seconds. It
// never writes zero: telling a caller it may retry immediately invites a retry
// that is certain to be refused again.
func SetRetryAfter(header http.Header, after time.Duration) {
	header.Set(retryAfterHeader, strconv.FormatInt(max(deltaSeconds(after), 1), 10))
}
