...
CREATE INDEX IF NOT EXISTS idx_password_reset_token_assistant
...
COMMENT ON INDEX public.idx_password_reset_token_assistant IS 'Issuing a link first deletes the assistant''s outstanding ones, so at most one is ever live. That delete keys on assistant_id and is the only reason this index exists.';
