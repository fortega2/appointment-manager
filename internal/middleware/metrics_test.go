...
func TestRouteTemplateMatchedAndUnmatched(t *testing.T) {
	t.Parallel()

	var matched *http.Request
	mux := newMux(patternAppointmentByID, func(_ http.ResponseWriter, r *http.Request) {
		matched = r
	})
	mux.ServeHTTP(httptest.NewRecorder(), httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/appointments/42", nil))
	require.NotNil(t, matched)
	assert.Equal(t, routeAppointmentByID, middleware.RouteTemplate(matched))

	unmatched := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/does-not-exist", nil)
	assert.Equal(t, "unmatched", middleware.RouteTemplate(unmatched))
}
